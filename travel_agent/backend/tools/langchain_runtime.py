"""用 LangChain create_agent 执行旅行客服只读业务工具闭环。"""
...
class OrderIdInput(BaseModel):
    order_id: str = Field(..., description="行程单号，必须来自用户输入或受控上下文")


class ProductQueryInput(BaseModel):
    keyword: str = Field(..., description="用户明确提到的套餐或目的地关键词，例如东京五日")
        ...
        *,
        ...
        def route_plan_mismatch(tool_name: str, order_id: str) -> ToolCallTrace | None:
            ...
            call = ToolCallTrace(
                ...
                output_summary="模型工具参数与受控 RoutePlan 的行程单号不一致，已阻止执行。",
                status="error",
                risk_level="high",
        ...
        def product_route_plan_mismatch(keyword: str) -> ToolCallTrace | None:
            ...
            call = ToolCallTrace(
                ...
                output_summary="模型工具参数与受控 RoutePlan 的套餐查询词不一致，已阻止执行。",
                status="error",
                risk_level="medium",
                next_action="ask_clarification",
                error_type="route_plan_argument_mismatch",
        ...
        tool_catalog: dict[str, BaseTool] = {
            "get_order_detail": StructuredTool.from_function(
                ...
                description="只读查询当前登录用户的行程详情；不能退票、取消或修改行程。",
            ...
            "get_order_logistics": StructuredTool.from_function(
                ...
                description="只读查询当前登录用户的行程与出行进度；不能修改值机或出行状态。",
                args_schema=OrderIdInput,
            ),
            "get_refund_status": StructuredTool.from_function(
                func=query_refund_status,
                name="get_refund_status",
                description="只读查询当前登录用户行程已经存在的退票申请状态；不能创建退票。",
                args_schema=OrderIdInput,
            ),
            "search_products": StructuredTool.from_function(
                func=query_products,
                name="search_products",
                description="按套餐或目的地关键词查询实时价格、可订名额和活动事实。",
        ...
        try:
            ...
            if expected_order_id:
                controlled_user_message += f"\n\n受控 RoutePlan 已确认行程单号：{expected_order_id}"
            if expected_product_keyword:
                controlled_user_message += f"\n\n受控 RoutePlan 已确认套餐查询词：{expected_product_keyword}"
            result = agent.invoke(
