...
class TraceEventNormalizer:
    ...
    _CATEGORY_BY_EVENT = {
        ...
        "booking_slots_blocked": "workflow",
        "booking_confirm_required": "workflow",
        "booking_slots_confirmed": "workflow",
    }
    _PHONE_PATTERN = re.compile(r"\b1[3-9]\d{9}\b")
    _ADDRESS_PATTERN = re.compile(r"(出行地址|入住酒店|地址)\s*[:：]\s*[^,，。;\n]+")
...
class TraceStore:
    def __init__(self) -> None:
        """初始化 Trace 存储，保证评测用例之间互不串扰。"""
        self._events: dict[str, list[TraceEvent]] = {}

    def clear(self) -> None:
        """清空测试会话 Trace，保证评测用例之间互不串扰。"""
        self._events.clear()
    ...
    *,
    ...
    trace_store.add(
        ...
        "model_route_planned",
        {
            ...
            "policy_tags": getattr(route_result, "policy_tags", []),
            "secondary_intents": getattr(route_result, "secondary_intents", []),
        },
    )
