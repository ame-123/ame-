...
结合 [session_state] 和 [recent_dialogue] 判断本轮是继续上一任务还是新话题。先看用户本句在说什么，再看状态，不要因为草稿还在就锁死预订。当前句往往只有两三个字，正确答案完全取决于历史，禁止只看本句。
- 客服刚列出预订清单并请确认，用户回「确认 / 确定 / 对的 / 好的 / 可以 / 行 / 嗯好 / 好的呢 / 知道啦 / 好呀 / 信息没问题」→ booking_request（这是在点头，不是闲聊）。
- 客服只是打招呼或问「需要查行程还是了解套餐」，用户回「好的 / 嗯 / 收到 / 好的呢 / 知道啦」→ general_chat。
- 报完价、客服没有请确认时，用户回「好的 / 收到」→ general_chat。
- 还在补目的地或日期，用户回「确认 / 那就确认吧？ / 我确认了吗 / 现在能确认不」或补「东京」「6月10号」→ booking_request（仍在订，缺槽由执行层再问）。
- 「先不订了 / 先这样 / 算了」即使草稿还在 → general_chat。
- 「刚才那单还是要 / 那单我还要」出现在用户刚放弃预订之后 → booking_request。
- 本句改问发票、行程、退票、价格、活动 → 按本句新意图走，不要黏住 recent_intent。
- 「审批过了吗 / 是不是已经送审了 / 人审过了没」在尚未提交预订时不是查询已有工单，不要走 refund_status_query。
